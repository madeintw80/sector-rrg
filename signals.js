...
(function () {
  ...
  const SCOREBOARD = [
    ...
    {
      name: "S 組：權值股 RRG（個股層）",
      guess: "同一套跨線訊號搬到市值前 150 大「個股」（母體每日重算）；加權＝市值加權、前 150 大＝拉動引擎，買「轉入領先＋確認」的權值股機械上才有機會贏加權",
      result: "全滅：I→L＋hot 44.5%／+0.26%／中位 −0.51%（n=555）、W→L＋hot 45.4%／+0.24%（n=945）；12 種跨線 × 加權／等權／前150等權三把尺，n≥100 的格子無一過 50%、跨年零例外、60 日更爛（39~40%）。附帶發現：前 150 檔等權三年 +105.6% vs 加權 +142.4%＝等權買整池權值股也輸 37pp，加權的引擎是市值加權本身（超級權值集中）",
      verdict: { cls: "dead", label: "❌ 死亡" },
      note: "2026-08-28 判死（research/rrg_stock_backtest.py，事前宣告制）；絕對報酬其實是正的（5 日 +1%）＝訊號抓得到會漲的股票、抓不到贏大盤的股票；個股離散讓中位數轉負＝題材等權指數才是這訊號的家，本頁三訊號不受影響",
    },
    {
      name: "T 組：大盤擇時線（risk-off 退場）",
      guess: "不挑股改抓時點：平時抱大盤（^TWII），廣度／輪動退潮訊號亮起退場 100% 現金，靠「少跌」贏 buy & hold；判準＝同窗 CAGR＋MDD（最大回撤）雙嚴格優",
      result: "11 格全滅：buy & hold 三年 CAGR +42.4%＝怪物牛市，最佳格（輪動佔比 <0.45）+29.9% 仍差 12pp；0% 成本照跑仍輸 ~3pp＝死因是「錯過反彈」不是成本。輪動佔比（150 檔 JdK 領先+改善比例）躲跌是真的——考窗三次 >10% 回撤全躲（最大一段少跌 25.7pp、MDD −17.7% vs −28.7%）但 CAGR 追不回；廣度水位家族反被 whipsaw 巴到比大盤慘（−37.9%），還輸給最笨的 60 日均線對照組",
      verdict: { cls: "dead", label: "❌ 死亡" },
      note: "2026-08-28 判死（research/market_timing_backtest.py，事前宣告制）；長窗附錄：^TWII 2000→2026 同款均線規則反而 CAGR＋MDD 雙優（6.62% vs 6.01%、−30% vs −66%）＝三年怪物牛市是擇時最嚴苛環境、擇時價值只在有大熊市的長窗兌現；輪動佔比降格為雷達「大盤健康度儀表」（純顯示、非交易訊號）",
    },
    {
      name: "U 組：早鳥規則化（點火前搶跑）",
      guess: "回推解剖案收官題：把 case-control 找到的分辨器寫成每天亮燈的規則——wl 早鳥（轉弱/領先象限＋RSI>60 或族群廣度>0.65）＋il far 型早鳥（改善象限＋RS 線與價雙站上 MA20），亮燈日全算（含假警報）提前 1~5 天進場，驗能不能贏加權",
      result: "24 格全滅：族群＋個股 × 2 規則 × 6 種出場（固定 5/10/20 日、動能回落 100、收破 MA20、移動停損 −10%）勝率 29.7~46.3% 全 <50、中位全負；跨年 72 個年格零格過 50＝連單年僥倖都沒有。出場菜單同場對決＝沒有一種出場救得了進場點（「固定天數是勝率上不去的原因」假設在此線否決）。雷達副產品：wl 點火前 92.8% 曾亮燈（覆蓋高）但亮燈後 5 日內真點火僅 5~21%＝假警報 80~95%",
      verdict: { cls: "dead", label: "❌ 死亡" },
      note: "2026-08-28 判死（research/rrg_early_bird_backtest.py，事前宣告制、對帳七重）；「提早進場」E1/E2/U 三度全滅＝蓋棺：訊號只能等確認、不能搶跑。分辨器（d≈0.6~1.2）是真訊息但蓋不過搶跑的結構性負超額；near 型（V 拉）續集已開完判死（見 V 組）；本頁三訊號不受影響",
    },
    {
      name: "V 組：near 型（V 拉）分辨器",
      guess: "回推解剖案續集：il 熱門確認約半數是「V 型急拉」（T-5 還在跌、既有 33 特徵全空白）——換兩個角度找分辨器：①確認日分型（run-up 起點回推、當下可判不開天眼）當現行訊號濾網 ②T-5 換特徵：族群傳染（族群先亮→個股補漲）＋量能乾涸（賣壓耗盡→V 拉）",
      result: "兩軌皆無活口：①分型濾網——族群層 near 在現行考卷尺（T+0 等權）54.1%／+0.69 反而是四個子集裡最好＝踢掉 V 拉會傷現行三訊號；個股層 near 確實拖累（排除後 T+0 加權 45.6→50.4%、兩把尺同向）但個股層 S 組本已判死、排除後主判準仍 46.6%<50＝濾網方向真實、救不活死線。②T-5 分辨器——傳染假說反向死亡：V 拉前 5 天所屬族群幾乎沒先亮（0.3% vs 對照 1.5%）、最強母艦自己也弱（d=−0.55）、同儕沒點火；量能乾涸方向命中且 near 專屬（d=−0.36、far 近零）但不到 0.5 中等線出不了規則",
      verdict: { cls: "dead", label: "❌ 死亡" },
      note: "2026-08-28 判死（research/rrg_near_type_probe.py，事前宣告制、對帳六重＋md5 重跑）；蓋棺：V 拉＝孤兒行情非族群補漲、分型確認日可判但無濾網增量；乾涸方向 near 專屬 d=−0.36 記檔不出規則。附帶畫面：族群 near 有群發日（單日 13 族群同時確認、發生在大盤下跌日）＝跨族群大搬風現象；本頁三訊號不受影響",
    },
  ];
