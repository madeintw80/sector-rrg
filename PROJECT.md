...
- `rrg_web.json`：schema v6 每日資料；保留 12／34／229 個三層互斥產業分類，另含受控 `topic_rrg`、1,970 家全市場公司索引、MoneyDJ 多標籤、逐層覆蓋率、5／20／60 日群組確認資料，以及 `company_prices` 的近 126 個交易日還原收盤價與成交量；前端採 network-first。
...
- schema v6 JSON 約 18.43 MiB；個股價量以 126 日共用時間軸、compact offset 與緊湊數值陣列輸出，前端只展開目前公司圖卡與最多 20 個題材。後續若手機實測載入變慢，再評估拆檔或按需載入。
